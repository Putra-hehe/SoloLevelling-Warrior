...
interface AuthPageProps {
  ...
  onOAuth: (provider: "google" | "facebook") => Promise<void>;
}
...
export function AuthPage({ onAuth, onOAuth }: AuthPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isBusy) return;
    if (!email || (!password && !isSignup)) return;
        ...
        <Card className="p-8 bg-card/50 backdrop-blur-sm border-border/50">
          ...
          <div className="mt-6 p-3 rounded-lg bg-secondary/30 border border-border/50">
            <p className="text-xs text-muted-foreground text-center">
              ...
              <br />
              Jika login di mobile gagal ("status awal hilang" / redirect error), coba buka di Chrome/Safari (bukan in-app browser seperti IG/FB) lalu coba lagi.
            </p>
          </div>
        </Card>
      </motion.div>
