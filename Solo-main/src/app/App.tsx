...
// --- OAuth helpers (mobile resilience) ---
const OAUTH_REDIRECT_FLAG_KEY = "levelday_oauth_redirect";
...
function isLikelyInAppBrowser(): boolean {
  ...
  // Common in-app browser identifiers (Instagram, Facebook, Line, etc.)
  return /FBAN|FBAV|Instagram|Line\/|KAKAOTALK|wv\b|WebView/i.test(ua);
}
...
export default function App() {
  ...
  // Track auth UID explicitly so we can safely gate Firestore reads/writes.
  // (Mobile browsers / private mode can drop persistence and cause permission errors.)
  ...
  const handleOAuth = async (providerName: "google" | "facebook") => {
    try {
      ...
      /**
       * MOBILE FIX:
       * Many mobile browsers (especially iOS Safari / in-app browsers) can lose
       * `sessionStorage` during the OAuth redirect round-trip, causing:
       *   "missing initial state" / auth/missing-initial-state
       * So we prefer POPUP first (works when triggered by a user tap).
       * If popup is blocked, we fall back to REDIRECT only when storage is available
       * and we're not in a known in-app browser.
       */
      const inApp = isLikelyInAppBrowser();
      const canRedirect = storageAvailable("sessionStorage") && !inApp;
      ...
      try {
        ...
        const popupCode = popupErr?.code as string | undefined;

        // If popup is blocked/unavailable, try redirect as a fallback.
        if (
          ...
          if (canRedirect) {
            // Mark that we initiated a redirect, so we can show a better message
            // if getRedirectResult later fails on mobile.
            try {
              if (storageAvailable("localStorage")) {
                localStorage.setItem(OAUTH_REDIRECT_FLAG_KEY, providerName);
              }
            } catch {
              // ignore
            }
          ...
          toast.error("Login tidak didukung di browser ini", {
            description: inApp
              ? "Kamu sedang membuka dari in-app browser (IG/FB/TikTok, dll). Buka link ini di Chrome/Safari lalu coba lagi."
              : "Popup login diblokir. Izinkan popup atau coba browser lain.",
          });
          return;
        }

        // Unknown popup error → bubble up to the common handler.
        throw popupErr;
      }
    } catch (err: any) {
      ...
      if (code === "auth/unauthorized-domain") {
        ...
        toast.error("Login redirect gagal di mobile", {
          description:
            "Browser kamu menghapus state login (sessionStorage) saat redirect. Coba login lagi (kami pakai mode popup), atau buka di Chrome/Safari (bukan in-app browser).",
        });
      } else {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // auth/missing-initial-state: common on mobile when sessionStorage is not
        // preserved through the redirect round-trip (Safari / in-app browsers).
        if (code === "auth/missing-initial-state") {
          toast.error("Login redirect gagal di mobile", {
            description:
              "Browser kamu menghapus state login saat redirect. Coba login lagi (kami pakai mode popup), atau buka di Chrome/Safari (bukan in-app browser).",
          });
        } else {
  ...
  useEffect(() => {
    ...
    // Gate Firebase hydration to only run when the Firebase Auth user matches.
    // Prevents "Missing or insufficient permissions" spam on mobile when
    // local state exists but auth session was not restored.
  ...
  const handleAuth = async (
    ...
    try {
      ...
      const code = err?.code as string | undefined;
      const message =
        code === "auth/invalid-login-credentials" || code === "auth/invalid-credential" || code === "auth/wrong-password" || code === "auth/user-not-found"
          ? "Email atau password salah."
          : code === "auth/email-already-in-use"
          ? "Email ini sudah terdaftar. Coba Sign In."
        : code === "auth/too-many-requests"
          ? "Terlalu banyak percobaan login. Coba lagi beberapa saat atau reset password."
        ...
        : code === "auth/network-request-failed"
          ? "Jaringan bermasalah. Cek koneksi internet lalu coba lagi."
  ...
  return (
    // Use dynamic viewport units when supported to prevent iOS/Android
    // browser chrome from causing 100vh layout glitches.
    <div className="dark min-h-screen min-h-[100dvh] bg-background text-foreground">
      {isAppPage ? (
        <div className="flex overflow-x-hidden">
          {/* Mobile Topbar */}
          <div className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between border-b bg-background/80 backdrop-blur px-4 py-3 pt-[calc(0.75rem+env(safe-area-inset-top))] md:hidden">
          ...
          {/* Main Content */}
          <div className="flex-1 min-w-0 min-h-screen min-h-[100dvh] pb-20 pb-[calc(5rem+env(safe-area-inset-bottom))] md:pb-0">
            <div className="container mx-auto max-w-7xl px-4 py-4 pt-16 pt-[calc(4rem+env(safe-area-inset-top))] sm:px-6 sm:py-6 md:pt-6">
